...
var (
	...
	serveCookieDomain = serve.Flag("cookie-domain", "Domain to set cookie for. If not, it is deduced from Host header.").String()
	serveCookieSecure = serve.Flag("cookie-secure", "Whether domains should have 'secure' attribute or not. If not defined, it's disabled if --insecure is given, otherwise enabled.").Bool()
